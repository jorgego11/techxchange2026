...
Finally:
* Replace the `<concert VM fqdn>` with the concert VM fully qualified domain 
name value from **credentials.sh** file
...
### 4.2.3 Execution authentication (non-FaaS) `concert_ansible_auth`
...
- **Inventory:** leave empty but make sure to check the **Overridable** checkbox!
...
## 4.3 Configure the workflow inputs
...
4. Instead of manually adding Concert's public key into the liberty VM ~/.ssh/authorized_keys
from a Terminal window, run the following command by replacing `liberty_public_ip` with the actual 
value from credentials.sh:

```bash
ssh-copy-id -f -i ~/.ssh/id_rsa.pub -p 2223 -o IdentityFile=$HOME/ssh_liberty_key.pem itzuser@<liberty_public_ip>
```
...
5. Click **Next**.
6. Under **Select a software offering**, choose **IBM WebSphere**, then
   **Next**.
...
Registration by IP address is not supported in this release
:::
...
## 4.9 Review the detected vulnerabilities
...
1. In Concert, click the **Resilience** tab on top and select **Dimensions → Vulnerability**
on the left menu
