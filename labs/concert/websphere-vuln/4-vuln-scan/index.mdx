...
register servers, retrieve advisories, assess vulnerabilities, and remediate.

1. In the Concert UI, go to **Workflows**.
2. Click **Create workflow → Select from library**.
3. Search for `websphere` and import each of the WebSphere vulnerability
   management workflows, **one at a time**.
...
| Create Change Request For Remediation Action | Create_Change_Request_For_Remediation_Action | Creates change requests when external approval is used. |
...
:::caution
This lab uses the **non-FaaS** path. When you search the library you will also
see `_Faas` variants of some workflows (for example
`Onboard_WebSphere_Server_Faas`). **Skip every `_Faas` workflow.** Importing the
wrong execution model will cause failures later.
:::

:::note
When you import **Websphere_Remediation_Master**, Concert automatically imports
the dependent remediation workflows it needs, including
`Websphere_Remediation_Main`, `Websphere_Remediation_Fix_Download`, and
`Websphere_Remediation_Fix_Install_Uninstall`. You do not import those
separately.
:::
...
1. In the Concert UI, open the navigation menu.
2. Go to **Workflows → Authentications**.
3. Click **Create authentication**.
4. Fill in the values shown below.
5. Save.
...
- **Service:** `Config Data`
- **Data:** provide the following JSON, using your instance's values:

```json
{
  "host": "concert-v3-auto71.fyre.ibm.com",
  "concert_port": "12443",
  "ssh_port": "22",
  "instanceId": "0000-0000-0000-0000",
  "Authorization": "<Concert_API_key>",
  "private_key": "<base64_encoded_private_key>",
  "public_key": "<public_key>"
}
```

To generate the key values, run these commands **on the Concert host**:

```bash
# Base64-encoded private key for the "private_key" field
base64 -w 0 ~/.ssh/id_rsa

# Public key for the "public_key" field
cat ~/.ssh/id_rsa.pub
```
...
3. Leave `sm_auth` empty (this lab does not use external approval).
4. Save.
