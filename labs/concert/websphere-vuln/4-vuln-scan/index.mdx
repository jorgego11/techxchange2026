...
3. Select **Downloads** on the right side, pick the file you just downloaded and click on **Open** to install the workflows
...
7. Set **Ends** to **Never**
8. Set the repeat interval from the table
9. Save
...
:::caution
Register WebSphere servers only **after** the initial **Websphere advisory job** run completes.
...
3. Click **Get public key**. Concert displays its public SSH key. We have already added Concert's public key into the Liberty VM ~/.ssh/authorized_keys
during the Lab Preparation section
