...
## 3.4 Enable Port Forwarding in Both VMs

Run the commands below on both, the Concert VM **Terminal** window and the
Liberty VM **Terminal** window:
...
In order to allow SSH connection between the Concert and Liberty we will
copy the each other SSH public keys.
